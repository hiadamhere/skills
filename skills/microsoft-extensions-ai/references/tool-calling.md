...
| `ChatToolMode.None` | Requests an answer without tool calls; adapter/model support is required. |
| `ChatToolMode.RequireAny` | Requests a call to **some** tool; adapter/model support is required. |
| `ChatToolMode.RequireSpecific("get_weather")` | Requests a call to **that** tool; adapter/model support is required. |
...
- **Keep `IncludeDetailedErrors = false` in production.** Enabling detailed errors can disclose exception text to the model and potentially to the user.
...
The rule below says tool arguments never carry authorization. Bind identity from trusted application context. Hiding a field from the schema alone is not authorization: the binding below overrides model-supplied values, and the tool must still enforce access checks:
...
- `UseFunctionInvocation()` is in the pipeline wherever local `AIFunction` tools must execute.
