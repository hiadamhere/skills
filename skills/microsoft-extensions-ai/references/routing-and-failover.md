...
- **It embeds the text of the most recent *user* message** — not the last message of any role, and not the system prompt. A nonempty request whose last user message has no text (or has no user message) goes to `defaultClient` without calling the embedding generator. An empty message list instead throws `InvalidOperationException` (retained T11n probe); validate input before routing.
- **Profile phrases are embedded lazily, once.** Nothing is embedded at construction; the first request embeds every phrase in a single batch and caches the result, then each subsequent request with user text makes one query-embedding call. The no-user/no-text cases above make none.
