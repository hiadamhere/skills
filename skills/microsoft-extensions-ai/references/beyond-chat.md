...
---
*Verified against Microsoft.Extensions.AI 10.9.0 DLL surface (`Microsoft.Extensions.AI` + `.Abstractions`) and compile-tested against the pinned package (2026-08-28): every code fence on this page compiles against 10.9.0 in the whole-skill fence harness, and every type on it is byte-identical to 10.8.1 by mechanical diff of the dumps (base types, parameter defaults, accessors and `[Experimental]` gating unchanged by a reflection shape diff). The optional `options`/`ServiceLifetime`/`CancellationToken` parameters, the `DataContent` convenience overloads on the speech and file clients, `AsBuilder()` on each family, and the `UseImageGeneration` chat-pipeline composition are compile-test facts a reflection dump renders identically to required ones. The `MEAI001` gate on this page's types was established on 10.9.0 by compile error and a reflection sweep of both assemblies for `[Experimental]`.*
