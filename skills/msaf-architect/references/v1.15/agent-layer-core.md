# 🧠 Agent Layer Core (v1.15)

The agent-layer public API is byte-identical to v1.14. Use [the v1.13 agent-layer guide](../v1.13/agent-layer-core.md) for agent construction, sessions, and workflow binding, and [the v1.14 agent-layer guide](../v1.14/agent-layer-core.md) for the asynchronous agent-mode contract (`GetModeAsync` / `SetModeAsync`), asynchronous message injection, and the approval-middleware changes.

## 🛠️ v1.15 engineering guidance
...
- Pin the agent, abstractions, and workflows packages to the same `1.15.0` version; the three ship in lockstep.
...
---
*Verified against MAF v1.15.0 DLL surface (2026-08-01). The agent-layer surface is byte-identical to v1.14 by mechanical diff.*
