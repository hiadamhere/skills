...
| Trap | Reality |
| --- | --- |
| Suppressing `MAAI001` for the whole v1.18 agent layer | The bypass members `EnableInvocableFunctionBypassing` and `UseInvocableFunctionBypassing`, plus the types `BackgroundAgentsProvider` and `BackgroundAgentsProviderOptions`, are gated. `AllowConcurrentInvocation` and `MaxAutoApprovalIterations` compile clean — scope the suppression to what needs it. |
...
*Experimental options-type recheck (2026-09-20): `probe-contract-metadata` executed against MAF 1.18.0 confirms the `MAAI001` experimental attribute on `BackgroundAgentsProviderOptions`. The adoption summary now includes the options type already called out above; other historical claims retain their original provenance.*
