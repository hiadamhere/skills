...
The loop-agent public API is byte-identical to v1.17. Use the construction, evaluator, termination, and iteration-limit patterns in [the v1.13 loop guide](../v1.13/agent-loops.md), and the engineering guidance in [the v1.14 loop guide](../v1.14/agent-loops.md).

## 🧭 Choosing the right primitive in v1.18
...
- A loop wrapped in a `ToolApprovalAgent` now has a second bound: the v1.18 auto-approval cap (see [Agent Skills, Files, and Tool Approval](agent-skills.md)). The two limit different things — iterations of the loop versus auto-approved re-invocations within one run — and neither substitutes for the other.
...
---
*Verified against MAF v1.18.0 DLL surface (2026-08-27). The loop-agent surface is byte-identical to v1.17 by mechanical diff.*
