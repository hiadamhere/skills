# 💾 State and Persistence (v1.18)

The scoped-state, checkpoint-store, and serialization surfaces are byte-identical to v1.17. Use [the v1.15 state guide](../v1.15/state-and-persistence.md) for `CheckpointManager.GetLatestCheckpointAsync` and the resume entry point, and [the v1.13 state guide](../v1.13/state-and-persistence.md) for stores and `ICheckpointStore<T>` implementations.
...
---
*Verified against MAF v1.18.0 DLL surface (2026-08-27). The checkpointing surface is byte-identical to v1.17 by mechanical diff; the nullable `CheckpointInfo?` return is a compile-test fact established against 1.17.0, whose Workflows surface v1.18 leaves unchanged. Workflow-hosting block added 2026-08-27: `AsAIAgent` executed against pinned 1.14.0, 1.17.0 and 1.19.0.*
