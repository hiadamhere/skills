# 🧰 Agent Skills, Files, and Tool Approval (v1.22)

The agent-skills system feeds reusable skill definitions (`AgentSkill`) into an agent's context through `AgentSkillsProvider`, an `AIContextProvider`. Sources and filters are **context-aware** — they receive an `AgentSkillsSourceContext` identifying the requesting agent and session. The skill-source and approval contracts carry forward; file search and guarded edits change in v1.21 as described below.
...
## File search and guarded edits (v1.21+)

`AgentFileStore.SearchAsync` is now virtual with a default implementation, rather than abstract. Existing overrides remain valid. Custom overrides must use the same line coordinates as the editor: use `AgentFileStore.SplitLines` and `AgentFileStore.ScanContent` rather than a different newline split.

```csharp
// Microsoft.Agents.AI 1.21.0 or 1.22.0; suppress MAAI001 in this project.
using System.Text.RegularExpressions;

IReadOnlyList<string> lines = AgentFileStore.SplitLines("a\r\nb\rc\nd");
FileSearchResult? result = AgentFileStore.ScanContent(
    "sample.txt", "a\r\nb\r\nc", new Regex("b$"));
var edit = new FileLineEdit { LineNumber = 2, ExpectedLine = "b", NewLine = "B\r\n" };
```

Executed on both pins: `SplitLines` preserves CRLF, LF and lone CR terminators; `ScanContent` matches the text without its terminator, returns 1-based `MatchingLines` with verbatim text, and returns null on no match. `FileLineEdit.ExpectedLine` is nullable and optional. The line editor rejects a stale value with `ArgumentException`; supply the line text rather than a numbered-read prefix. The replacement `NewLine` includes the desired terminator: omitting it joins the replacement to the following line. These types remain `MAAI001`-gated.

The package documentation specifies that a trailing terminator is ignored when comparing `ExpectedLine`. This is a stale-read check, not a transactional lock: a storage implementation still needs its own concurrency policy.
...
`DelegatingAgentSkillsSource` is the abstract base that forwards to an inner source, and exactly three concrete decorators derive from it and wrap any source, including your own. The last two rows below are **not** decorators: they derive from `AgentSkillsSource` directly — as does the `AgentFileSkillsSource` above — and take a collection rather than an `innerSource` (reflection surface).
...
*Correction verified against the `FileAccessProviderOptions.DisableWriteTools` package XML documentation for MAF 1.22.0 (2026-09-22); no new runtime-execution claim.*
