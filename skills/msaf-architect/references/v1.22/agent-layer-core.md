...
## Mode controls added in v1.22

`AgentModeProviderOptions.DisableModeSetTool` and `DisableModeGetTool` both default to false. Set them to true to keep mode changes under host control while omitting the corresponding model tools. Programmatic `GetModeAsync` and `SetModeAsync` still work; disabling both was executed and produced no mode tools in the provider context.

The new overload accepts `disableNotification`:

```csharp
await modeProvider.SetModeAsync(session, "write", disableNotification: true);
```

The mode change itself was executed. Per the package contract, `disableNotification: true` suppresses the mode-change message; it does not disable the change. These options and this overload are not `MAAI001`-gated (reflection and unsuppressed compilation). Mode definitions are the nested `AgentModeProviderOptions.AgentMode` type.
...
- Pin the agent, abstractions, and workflows packages to the same `1.22.0` version; the three ship in lockstep. MAF 1.22.0 requires `Microsoft.Extensions.AI` 10.10.0 or later.
