# 🚀 Production Readiness, Telemetry & Testing (v1.22)
...
## Default agent telemetry source (v1.22)

`OpenTelemetryAgent.DefaultSourceName` is a static property, gated by `MAAI001`; suppress that diagnostic where it is consumed. Pass it to the tracing pipeline's source subscription when the agent uses the default source. The value read on pinned 1.22.0 is `Experimental.Microsoft.Agents.AI`. Prefer the property to hardcoding that string: per the package contract it is read at runtime, so upgrades pick up the current default. This is the agent source, separate from the workflow source in the sample below.
...
4. Compile-test every version-specific snippet against exact `1.22.0` packages.
