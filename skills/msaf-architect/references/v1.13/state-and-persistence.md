# 💾 Workflow State & Persistence (v1.13)

Managing session values, variables, and checkpoint snapshots in MAF v1.13.x relies on scoped state and the dedicated checkpointing namespace. This layer is byte-identical to v1.12 — the samples below are unchanged.

---

## 🔄 Scoped State Model

MAF v1.13.x separates state into scopes to isolate executor contexts and prevent key collisions. State is access-controlled via `IWorkflowContext` passed to `HandleAsync`.
...
MAF v1.13.x features a formal checkpointing framework (`Microsoft.Agents.AI.Workflows.Checkpointing`).
...
---
*Verified against MAF v1.13.0 DLL surface (2026-07-07). The Workflows checkpointing layer is byte-identical to v1.12 (mechanical surface diff), so the v1.12 compile-tested samples apply unchanged.*
