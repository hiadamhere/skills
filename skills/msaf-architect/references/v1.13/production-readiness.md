# 🚀 Production Readiness, Telemetry & Testing (v1.13)

Deploying MAF v1.13.x to production involves configuring scoped Dependency Injection, setting up streaming runners, and updating test assertions to match the `Run` class models. This layer is byte-identical to v1.12 — the samples below are unchanged.

---

## 🔌 Dependency Injection (DI)

Because v1.13.x uses `ExecutorBinding`, DI registration typically binds executors to factory-based routes.
...
MAF v1.13.x supports real-time streaming of events (like token-by-token LLM output or superstep progress events) via `StreamingRun`.
...
public class TextAnalyzerTests
{
    ...
    public async Task HandleAsync_CountsWordsCorrectly()
    {
        ...
        var result = await executor.HandleAsync("Hello MAF 1.13", mockContext.Object, CancellationToken.None);
...
---
*Verified against MAF v1.13.0 DLL surface (2026-07-07). The Workflows layer is byte-identical to v1.12 (mechanical surface diff), so the v1.12 compile-tested samples apply unchanged.*
