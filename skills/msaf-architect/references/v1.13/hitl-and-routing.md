# 🔀 Workflow Routing & Human-in-the-Loop (v1.13)

MAF v1.13.x uses type-safe `ExecutorBinding` routing, declarative topology builders, and the native `RequestPort` HITL resumption model. This layer is byte-identical to v1.11/v1.12 — the samples below are unchanged.

---

## 🏗️ Orchestration Builders

Instead of manually mapping edges for common patterns, MAF v1.13.x provides dedicated builders via `AgentWorkflowBuilder`:
