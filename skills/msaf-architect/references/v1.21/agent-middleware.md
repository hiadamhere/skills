# 🧬 Agent Middleware and Routing (v1.21)
...
| Pinning `Microsoft.Extensions.AI` below 10.10.0 next to MAF 1.21.0 | This pin depends on 10.10.0; the routing client's base type lives there. Let the dependency float or pin at least 10.10.0. |
...
*Experimental-gate metadata recheck (2026-09-20): retained probe-agent-correctness, executed against MAF 1.21.0, confirms the experimental attribute on the background options type and the bypass property, and its absence on concurrent invocation. This supports the type/member distinction above without re-dating unrelated historical claims.*
