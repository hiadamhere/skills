...
Run that and the executor reports `InputTypes = System.String`, `OutputTypes = System.Int32`, and a `"hello"` input yields `5`. **Executed against pinned 1.20.0** (2026-09-03) in the catalog's private compile harness: the router was bound into a one-node workflow with `WithOutputFrom`, run with `"hello"`, and its single `WorkflowOutputEvent` read back as `5`; the run's status afterwards was `RunStatus.Idle`.
