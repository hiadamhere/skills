...
A workflow hosted as an agent (`AsAIAgent`, see [State and Persistence](state-and-persistence.md)) is observed through the *agent* surface — `RunAsync` / `RunStreamingAsync` and agent response updates — not through `WatchStreamAsync`. The event taxonomy above is what you get when you run the `Workflow` directly.
...
---
*Verified against MAF v1.19.0 DLL surface (2026-08-27). The 21-type event set is identical across v1.11–v1.19 by mechanical diff; the probe behind the v1.11 guide compiles unmodified against pinned 1.11.0, 1.14.0 and 1.17.0, and the 1.19.0 surface it targets is unchanged.*
