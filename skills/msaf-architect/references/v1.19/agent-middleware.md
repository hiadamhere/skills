# 🧬 Agent Middleware and Routing (v1.19)

Everything that wraps an agent's chat client, plus the agent-layer surfaces that manage work outside a single turn. For constructing the agent itself — sessions, bindings, modes — see [Agent Layer Core](agent-layer-core.md).
...
`BackgroundAgentsProvider` — experimental (`MAAI001`) in every version tested here, 1.17.0 through 1.19.0 — takes a session out of service:
...
The `MAAI001` gate can apply to **types or members**. For the middleware discussed here, the bypass members `EnableInvocableFunctionBypassing` and `UseInvocableFunctionBypassing`, plus the types `BackgroundAgentsProvider` and `BackgroundAgentsProviderOptions`, are gated; `AllowConcurrentInvocation` compiles clean. The auto-approval cap `MaxAutoApprovalIterations` is not gated either and lives in [Agent Skills, Files, and Tool Approval](agent-skills.md), where its `const` default of 40 is documented.
