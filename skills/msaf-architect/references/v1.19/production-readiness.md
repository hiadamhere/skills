...
`AddOtlpExporter` comes from the `OpenTelemetry.Exporter.OpenTelemetryProtocol` package, which MAF does not pull in for you; `Sdk` is `OpenTelemetry.Sdk`, so the first `using` is not optional. Compile-tested with `OpenTelemetry.Exporter.OpenTelemetryProtocol` 1.15.3, which matches the `OpenTelemetry.Api` version the 1.19.0 packages declare in their nuspec (the compile itself ran against pinned 1.20.0; the Workflows surface is byte-identical between the two).
