...
Compaction operates on the agent's session history. With a session-routed chat client (see [Agent Layer Core](agent-layer-core.md)) that history is shared across routes by design, so a compaction decision made while one route was active is what the next route sees — one more reason the routes must all use client-side history.
...
---
*Verified against MAF v1.19.0 DLL surface (2026-08-27). The compaction namespace is byte-identical across v1.10–v1.19 by mechanical diff; the probe program behind the v1.11 guide compiles unmodified against pinned 1.11.0, 1.14.0 and 1.17.0, and the 1.19.0 surface it targets is unchanged.*
