...
The loop-agent public API is byte-identical to v1.18. Use the construction, evaluator, termination, and iteration-limit patterns in [the v1.13 loop guide](../v1.13/agent-loops.md), and the engineering guidance in [the v1.14 loop guide](../v1.14/agent-loops.md).

## 🧭 Choosing the right primitive in v1.19
...
A workflow that must be driven like an agent — sessions, `RunAsync`, serialization — is hosted with `AsAIAgent`; see [State and Persistence](state-and-persistence.md) for the chat-protocol requirement and the v1.19 checkpoint controls that come with it.
...
---
*Verified against MAF v1.19.0 DLL surface (2026-08-27). The loop-agent surface is byte-identical to v1.18 by mechanical diff.*
