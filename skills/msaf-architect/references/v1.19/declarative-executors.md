...
> [!WARNING]
> **`ReflectingExecutor<TExecutor>` is `[Obsolete]` here too** — confirmed present in 1.11.0, 1.14.0, 1.17.0, 1.18.0 and 1.19.0. It compiles and runs, so only a **CS0618** warning marks it. No surface dump can show this: the analyzer emits members, not the attributes on a type — which is also why a byte-identical surface between two versions cannot carry the fact forward, and why it was re-checked by compile test on 1.19.0.

The v1.11 guide's traps are unchanged in v1.19:
...
The 1.19.0 packages ship no `analyzers/` folder either — the generator the obsoletion message promises is still absent.

One fact — true in every verified version, and relevant here because v1.19 adds the checkpoint controls for hosted agents — matters when an executor's workflow is to be hosted as an agent: the protocol an executor declares is what `AsAIAgent` checks at run time. A start executor whose protocol does not accept `List<ChatMessage>` and `TurnToken` makes the hosted agent throw on its first run — see [State and Persistence](state-and-persistence.md).
...
---
*Verified against MAF v1.19.0 DLL surface (2026-08-27). The `[Obsolete]` marker on `ReflectingExecutor` (CS0618) was re-confirmed by compile test against the pinned 1.19.0 packages; the `protected abstract ConfigureProtocol` (CS0534) is invisible to a reflection dump and carried by the compile-verified register.*
